...
def stats_table_import():
    ...
    with sqlite3.connect(args.destination) as dest_db:
        ...
        for row in temp_list:
            ...
            c.execute('INSERT INTO "stats" ("author" , "month_num" , "day_num" , "day_week" , "year" , "hour") VALUES (?, ?, ?, ?, ?, ?)', (row[0], row[1], row[2], row[3], row[4], row[5]))
        dest_db.commit()
        c.close()
...
def quote_table_import():
    ...
    with sqlite3.connect(args.destination) as dest_db:
        ...
        for row in temp_list:
            ...
            c.execute('INSERT INTO quotes ("quote","time","author","author_id") VALUES (?, ?, ?, ?)', row[0], row[1], row[2], row[3])
        dest_db.commit()
        c.close()
